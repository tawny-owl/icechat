...
var IceComm = function(APIKEY) {

  var rooms = {};
  var eventsObj = {};
  var specialEventsObj = {};
  var socket = io.connect('http://icecomm.cloudapp.net:80');
  ...
  function gotStreamSuccess (stream) {
    ...
    if (specialEventsObj.local) {
      var options = {
        stream: URL.createObjectURL(stream)
      };
      triggerSpecialEvent('local', options);
  ...
  socket.on('offer', function(descriptionObj) {
    ...
    if (myID === descriptionObj.to) {
      ...
      createPeerConnections(callerID);
      localPeerConnections[callerID].addStream(localStream);
      localPeerConnections[callerID].setRemoteDescription(new RTCSessionDescription(descriptionObj.description));

      localPeerConnections[callerID].createAnswer(function(description) {
  ...
  socket.on('candidate', function(candidateObj) {
    ...
    if (myID === candidateObj.to) {
      ...
      localPeerConnections[callerID].addIceCandidate(candidate);
    }
  });
  ...
  function createPeerConnections(callerID) {
    console.log('creating PC', RTCPeerConnection);
    localPeerConnections[callerID] = new RTCPeerConnection(serverInfo, optionalRtpDataChannels);

    dataChannel[callerID] = localPeerConnections[callerID].createDataChannel('RTCDataChannel', {reliable: false});
    ...
    // localPeerConnections[callerID].onnegotiationneeded = negotiationneeded;
    localPeerConnections[callerID].onaddstream = function gotRemoteStream(event){
      remoteStreams[callerID] = event.stream;
    };

    localPeerConnections[callerID].onnegotiationneeded = function() {
      console.log('negotation needed');
    }
  }

  function createDescription(description, callerID, offerOrAnswer) {
    localPeerConnections[callerID].setLocalDescription(description);
    var descriptionObj = {
      description: description,
      from: myID,
      to: callerID
    };
    console.log('before', offerOrAnswer);
  ...
  function handleData(data) {
    var room = data.room;
    var callerID = data.myID;
    var options = data.options;
    options.video = options.video || true;
    options.audio = options.audio || true;
    options.data = options.data || "";
    trigger(room, callerID, options);
  }

  function trigger(room, callerID, options) {
    options.stream = URL.createObjectURL(remoteStreams[callerID]);
    eventsObj[room](options);
  }

  this.sendData = function(data) {
    for (var key in localPeerConnections) {
      dataChannel[key].send(data);
    }
  }

  // needs to handle room stuff
  this.emit = function(room, options) {
    var data = {};
    data.myID = myID;
    data.room = room;
  ...
  this.on = function(room, callback) {
    eventsObj[room] = callback;
  }

  this.getVideo = function() {
    return URL.createObjectURL(localStream);
  }

  this.local = function(callback) {
    specialEventsObj.local = callback;
  }

  this.ready = function(callback) {
    specialEventsObj.ready = callback;
  }

  function triggerSpecialEvent(event, options) {
    if (event === 'ready') {
      options = {};
      options.remoteStreams = remoteStreams;
    }
    specialEventsObj[event](options);
